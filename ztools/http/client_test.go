...
// when not empty.
//
// ztls.Config.ServerName (non-empty, set to "example.com") takes
...
func TestResponseSetsTLSConnectionState(t *testing.T) {
	...
	tr := newTLSTransport(t, ts)
	tr.TLSClientConfig.CipherSuites = []uint16{ztls.TLS_RSA_WITH_3DES_EDE_CBC_SHA}
	tr.Dial = func(netw, addr string) (net.Conn, error) {
		return net.Dial(netw, ts.Listener.Addr().String())
	}
	defer tr.CloseIdleConnections()
	...
	if got, want := res.TLS.CipherSuite, uint16(ztls.TLS_RSA_WITH_3DES_EDE_CBC_SHA); got != want {

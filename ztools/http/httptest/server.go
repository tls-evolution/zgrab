...
func (s *Server) StartTLS() {
	...
	cert, err := ztls.X509KeyPair(http.LocalhostCert, http.LocalhostKey)
	if err != nil {
	...
	if existingConfig != nil {
		...
		s.TLS = new(ztls.Config)
	}
	if s.TLS.NextProtos == nil {
		s.TLS.NextProtos = []string{"http/1.1"}
	}
	if len(s.TLS.Certificates) == 0 {
		s.TLS.Certificates = []ztls.Certificate{cert}
	}
	s.Listener = ztls.NewListener(s.Listener, s.TLS)

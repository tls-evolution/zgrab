...
func TestNextProtoUpgrade(t *testing.T) {
	...
	ts.Config.TLSNextProto = map[string]func(*Server, *ztls.Conn, Handler){
	...
	// It is HTTP/0.9 over TLS.
	{
		...
		conn, err := ztls.Dial("tcp", ts.Listener.Addr().String(), tlsConfig)
		if err != nil {
			t.Fatal(err)
		}
...
func handleTLSProtocol09(srv *Server, conn *ztls.Conn, h Handler) {

...
type Transport struct {
	...
	// automatically.
	TLSNextProto map[string]func(authority string, c *ztls.Conn) RoundTripper
...
func (t *Transport) dialConn(ctx context.Context, cm connectMethod) (*persistConn, error) {
	...
	if tlsDial {
		...
		if tc, ok := pconn.conn.(*ztls.Conn); ok {
			...
			if err := tc.Handshake(); err != nil {
				...
				if trace != nil && trace.TLSHandshakeDone != nil {
					trace.TLSHandshakeDone(ztls.ConnectionState{}, err)
				}
				return nil, err
			}
	...
	if cm.targetScheme == "https" && !tlsDial {
		...
		tlsConn := ztls.Client(plainConn, cfg)
	...
	if s := pconn.tlsState; s != nil && s.NegotiatedProtocolIsMutual && s.NegotiatedProtocol != "" {
		if next, ok := t.TLSNextProto[s.NegotiatedProtocol]; ok {
			return &persistConn{alt: next(cm.targetAddr, pconn.conn.(*ztls.Conn))}, nil
		}
	}
...
// client or server.
func cloneTLSConfig(cfg *ztls.Config) *ztls.Config {
	if cfg == nil {
		return &ztls.Config{}
	}
	return cfg.Clone()

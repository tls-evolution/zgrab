...
func TestTransportDialTLS(t *testing.T) {
	...
	tr := &Transport{
		DialTLS: func(netw, addr string) (net.Conn, error) {
			...
			didDial = true
			mu.Unlock()
			c, err := ztls.Dial(netw, addr, &ztls.Config{
				InsecureSkipVerify: true,
			})
			if err != nil {
				return nil, err
...
func TestTransportAutomaticHTTP2_TLSNextProto(t *testing.T) {
	testTransportAutoHTTP(t, &Transport{
		TLSNextProto: make(map[string]func(string, *ztls.Conn) RoundTripper),
	}, false)
}

func TestTransportAutomaticHTTP2_TLSConfig(t *testing.T) {
	testTransportAutoHTTP(t, &Transport{
		TLSClientConfig: new(ztls.Config),
	}, false)
}
...
func TestNoCrashReturningTransportAltConn(t *testing.T) {
	cert, err := ztls.X509KeyPair(LocalhostCert, LocalhostKey)
	if err != nil {
		t.Fatal(err)
	}
	ln := newLocalListener(t)
	...
	go func() {
		tln := ztls.NewListener(ln, &ztls.Config{
			NextProtos:   []string{"foo"},
			Certificates: []ztls.Certificate{cert},
		})
		sc, err := tln.Accept()
		if err != nil {
			t.Error(err)
		...
		if err := sc.(*ztls.Conn).Handshake(); err != nil {
			t.Error(err)
			return
		}
		<-testDone
	...
	tr := &Transport{
		DisableKeepAlives: true,
		TLSNextProto: map[string]func(string, *ztls.Conn) RoundTripper{
			"foo": func(authority string, c *ztls.Conn) RoundTripper {
		...
		DialTLS: func(_, _ string) (net.Conn, error) {
			tc, err := ztls.Dial("tcp", addr, &ztls.Config{
				InsecureSkipVerify: true,
				NextProtos:         []string{"foo"},
			})
			if err != nil {
...
func testTransportEventTrace(t *testing.T, h2 bool, noHooks bool) {
	...
	if h2 {
		...
		trace.TLSHandshakeDone = func(s ztls.ConnectionState, err error) {
...
func TestTLSHandshakeTrace(t *testing.T) {
	...
	trace := &httptrace.ClientTrace{
		...
		TLSHandshakeDone: func(s ztls.ConnectionState, err error) {
			mu.Lock()
			defer mu.Unlock()
			done = true
			if err != nil {
	...
	tr := &Transport{TLSClientConfig: &ztls.Config{InsecureSkipVerify: true}}
	defer tr.CloseIdleConnections()
	c := MakeNewClient()
	c.Transport = tr
...
// know the successful ztls.Dial from DialTLS will need to go into the
...
func TestIdleConnH2Crash(t *testing.T) {
	...
	cst.tr.DialTLS = func(network, addr string) (net.Conn, error) {
		c, err := ztls.Dial(network, addr, &ztls.Config{
			InsecureSkipVerify: true,

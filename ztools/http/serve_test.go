...
func TestHTTP2WriteDeadlineExtendedOnNewRequest(t *testing.T) {
	...
	ts := httptest.NewUnstartedServer(HandlerFunc(func(res ResponseWriter, req *Request) {}))
	ts.Config.WriteTimeout = 250 * time.Millisecond
	ts.TLS = &ztls.Config{NextProtos: []string{"h2"}}
	ts.StartTLS()
	defer ts.Close()
...
func TestAutomaticHTTP2_Serve_NonH2TLSConfig(t *testing.T) {
	testAutomaticHTTP2_Serve(t, &ztls.Config{}, false)
}

func TestAutomaticHTTP2_Serve_H2TLSConfig(t *testing.T) {
	testAutomaticHTTP2_Serve(t, &ztls.Config{NextProtos: []string{"h2"}}, true)
}

func testAutomaticHTTP2_Serve(t *testing.T, tlsConf *ztls.Config, wantH2 bool) {
	setParallel(t)
	defer afterTest(t)
...
func TestAutomaticHTTP2_Serve_WithTLSConfig(t *testing.T) {
	...
	// *ztls.Config given to ztls.NewListener.
	s.TLSConfig = &ztls.Config{
...
func TestAutomaticHTTP2_ListenAndServe(t *testing.T) {
	...
	testAutomaticHTTP2_ListenAndServe(t, &ztls.Config{
...
func TestAutomaticHTTP2_ListenAndServe_GetCertificate(t *testing.T) {
	...
	testAutomaticHTTP2_ListenAndServe(t, &ztls.Config{
		GetCertificate: func(clientHello *ztls.ClientHelloInfo) (*ztls.Certificate, error) {
			return &cert, nil
		},
	})
}
...
func testAutomaticHTTP2_ListenAndServe(t *testing.T, tlsConf *ztls.Config) {
...
Try:
	...
	defer ln.Close()
	c, err := ztls.Dial("tcp", ln.Addr().String(), &ztls.Config{
		InsecureSkipVerify: true,

...
func (c *conn) serve(ctx context.Context) {
	...
	if tlsConn, ok := c.rwc.(*ztls.Conn); ok {
		if d := c.server.ReadTimeout; d != 0 {
		...
		c.tlsState = new(ztls.ConnectionState)
...
type Server struct {
	...
	TLSConfig *ztls.Config // optional TLS config, used by ListenAndServeTLS
	...
	// automatically.
	TLSNextProto map[string]func(*Server, *ztls.Conn, Handler)
...
func (srv *Server) shouldConfigureHTTP2ForServe() bool {
	if srv.TLSConfig == nil {
		...
		// ztls.NewListener and passed that listener to Serve.
		// So we should configure HTTP/2 (to set up srv.TLSNextProto)
		// in case the listener returns an "h2" *ztls.Conn.
		return true
	}
	// The user specified a TLSConfig on their http.Server.
	// In this, case, only configure HTTP/2 if their ztls.Config
	// explicitly mentions "h2". Otherwise http2.ConfigureServer
	// would modify the ztls.Config to add it, but they probably already
	// passed this ztls.Config to ztls.NewListener. And if they did,
...
//	}
//
// One can use generate_cert.go in crypto/ztls to generate cert.pem and key.pem.
//
...
func (srv *Server) ListenAndServeTLS(certFile, keyFile string) error {
	...
	if !configHasCert || certFile != "" || keyFile != "" {
		var err error
		config.Certificates = make([]ztls.Certificate, 1)
		config.Certificates[0], err = ztls.LoadX509KeyPair(certFile, keyFile)
		if err != nil {
			return err
		}
	}
	...
	tlsListener := ztls.NewListener(tcpKeepAliveListener{ln.(*net.TCPListener)}, config)

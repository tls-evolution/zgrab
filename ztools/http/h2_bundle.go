...
// c is never closed.
func (p *http2clientConnPool) addConnIfNeeded(key string, t *http2Transport, c *ztls.Conn) (used bool, err error) {
	p.mu.Lock()
...
func (c *http2addConnCall) run(t *http2Transport, key string, tc *ztls.Conn) {
...
func http2configureTransport(t1 *Transport) (*http2Transport, error) {
	...
	if t1.TLSClientConfig == nil {
		t1.TLSClientConfig = new(ztls.Config)
	}
	...
	upgradeFn := func(authority string, c *ztls.Conn) RoundTripper {
	...
	if m := t1.TLSNextProto; len(m) == 0 {
		t1.TLSNextProto = map[string]func(string, *ztls.Conn) RoundTripper{
			"h2": upgradeFn,
		}
	} else {
		m["h2"] = upgradeFn
...
func http2isBadCipher(cipher uint16) bool {
	switch cipher {
	case ztls.TLS_RSA_WITH_RC4_128_SHA,
...
func http2cloneTLSConfig(c *ztls.Config) *ztls.Config { return c.Clone() }
...
type http2connectionStater interface {
	ConnectionState() ztls.ConnectionState
}
...
func http2ConfigureServer(s *Server, conf *http2Server) error {
	...
	if s.TLSConfig == nil {
		s.TLSConfig = new(ztls.Config)
	} else if s.TLSConfig.CipherSuites != nil {
		// If they already provided a CipherSuite list, return
		// an error if it has a bad order or is missing
		// ECDHE_RSA_WITH_AES_128_GCM_SHA256.
		const requiredCipher = ztls.TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256
		haveRequired := false
		sawBad := false
	...
	if s.TLSNextProto == nil {
		s.TLSNextProto = map[string]func(*Server, *ztls.Conn, Handler){}
	}
	protoHandler := func(hs *Server, c *ztls.Conn, h Handler) {
...
// client. If c has a ConnectionState method like a *ztls.Conn, the
...
func (s *http2Server) ServeConn(c net.Conn, opts *http2ServeConnOpts) {
	...
	if tc, ok := c.(http2connectionStater); ok {
		sc.tlsState = new(ztls.ConnectionState)
		*sc.tlsState = tc.ConnectionState()

		if sc.tlsState.Version < ztls.VersionTLS12 {
...
type http2serverConn struct {
	...
	tlsState         *ztls.ConnectionState       // shared by all handlers, like net/http
...
func (sc *http2serverConn) newWriterAndRequestNoBody(st *http2stream, rp http2requestParam) (*http2responseWriter, *Request, error) {
	...
	var tlsState *ztls.ConnectionState // nil if not scheme https
	if rp.scheme == "https" {
...
type http2Transport struct {
	...
	//
	// If DialTLS is nil, ztls.Dial is used.
	//
	// If the returned net.Conn has a ConnectionState method like ztls.Conn,
	// it will be used to set http.Response.TLS.
	DialTLS func(network, addr string, cfg *ztls.Config) (net.Conn, error)

	// TLSClientConfig specifies the TLS configuration to use with
	// ztls.Client. If nil, the default configuration is used.
...
type http2ClientConn struct {
	t         *http2Transport
	tconn     net.Conn              // usually *ztls.Conn, except specialized impls
	tlsState  *ztls.ConnectionState // nil only for specialized impls
...
func (t *http2Transport) newTLSConfig(host string) *ztls.Config {
	cfg := new(ztls.Config)
...
func (t *http2Transport) dialTLS() func(string, string, *ztls.Config) (net.Conn, error) {
	if t.DialTLS != nil {
		return t.DialTLS
	}
...
func (t *http2Transport) dialTLSDefault(network, addr string, cfg *ztls.Config) (net.Conn, error) {
	cn, err := ztls.Dial(network, addr, cfg)
	if err != nil {
		return nil, err
	}
